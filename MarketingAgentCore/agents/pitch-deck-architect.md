...
2. **Automated Slide Deck Generation (.pptx):**
   - Direct execution of Python scripts using `python-pptx` via the workflow established in `MarketingAgentCore/skills/pitch-deck-creator/`.
...
- **Reference Context:**
  ...
  - `MarketingAgentCore/context/brand-visual-style-guide.md`
...
- Rendered Presentation Files (.pptx): `MarketingAgentCore/outputs/decks/`
...
3. When requested to generate physical presentations, write and execute Python scripts using `python-pptx` following `MarketingAgentCore/skills/pitch-deck-creator/`, outputting `.pptx` files to `MarketingAgentCore/outputs/decks/`.
