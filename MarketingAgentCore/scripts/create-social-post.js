...
const SOP_TEMPLATE_PATH = path.join(ROOT_DIR, 'MarketingAgentCore', 'templates', 'content-production-sop-template.md');
...
Ejemplo:
  bash MarketingAgentCore/scripts/create-social-post.sh instagram "capa-akatsuki-confeccion" encargo-saga "Naruto / Akatsuki" "Capa de Autor"
`);
    process.exit(1);
  }
  ...
  if (fs.existsSync(targetPath)) {
    ...
    console.error(`   bash MarketingAgentCore/scripts/refine-note.sh refine "${path.relative(ROOT_DIR, targetPath)}" "Tu resumen"\n`);
    process.exit(1);
  }
