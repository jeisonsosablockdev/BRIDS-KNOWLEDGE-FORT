...
TEMPLATE_PATH="$ROOT/MarketingAgentCore/templates/task-tracking-template.json"
...
if [[ -z "$SESSION_NAME" ]]; then
  echo "Uso: bash MarketingAgentCore/scripts/init-task.sh <session-name-kebab-case> [objetivo] [icp]"
  echo "Ejemplo: bash MarketingAgentCore/scripts/init-task.sh b2b-campaign-q3 '50 leads en 30 días' 'Directores de Marketing SaaS'"
  exit 1
fi
