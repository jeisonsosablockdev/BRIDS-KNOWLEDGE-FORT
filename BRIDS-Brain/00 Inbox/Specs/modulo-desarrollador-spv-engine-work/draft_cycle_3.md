...
Actualmente, la base de datos de BRIDS en GitHub dispone de perfiles individuales de usuario (`user_profiles`, migración `012`) enfocados en inversionistas y autenticación personal vía wallet o WorkOS. **La pieza que no existe en el sistema y que debemos crear es la entidad corporativa de la Promotora Inmobiliaria (`developer_entities`).**

Este nuevo perfil desacopla a la persona física (Manager/CFO) de la persona jurídica (empresa constructora o promotora) y centraliza toda la gobernanza B2B.

```mermaid
flowchart TD
    subgraph Individual["Capa Individual (Existente)"]
        UP["user_profiles (Migración 012)
• wallet_public_key
• KYC Stripe Identity
• Datos personales"]
    end

    subgraph Corporate["Capa Corporativa Promotora (NUEVA: developer_entities)"]
        DE["Perfil Tipo Promotora (Developer Entity)
• Razón Social y Nombre Comercial
• EIN Encriptado y Registro Estatal (Sunbiz/SOS)
• Operating Agreement y Documentos Constitutivos
• Wallet Institucional de Firma Squads
• Track Record, Biografía y Especialidad"]
    end

    subgraph Projects["Capa de Activos (Existente)"]
        ME1["marketplace_entries: Proyecto 104 Oak St"]
        ME2["marketplace_entries: Proyecto 220 Pine Ave"]
    end

    UP -->|Es Manager / Firmante de| DE
    DE -->|Origina y Administra| ME1
    DE -->|Origina y Administra| ME2
```

### A. Estructura de Datos del Perfil Tipo Promotora
El perfil corporativo de la promotora se estructura en 5 bloques funcionales:

1. **Datos de Identidad Mercantil:**
   - Razón social (*Legal Business Name*) y nombre comercial público.
   - Estado de incorporación (*State of Registration*: Florida, Texas, Delaware, etc.).
   - Número de Identificación Patronal (*EIN / Tax ID*) protegido mediante encriptación.
   - Enlace y folio de inscripción en la autoridad corporativa estatal (ej. *Sunbiz Division of Corporations*).
2. **Representación Legal & Gobierno Interno:**
   - Nombre y correo del Administrador (*Operating Manager* / Firmante Principal).
   - Clave foránea hacia su cuenta personal verificada (`manager_wallet` $ightarrow$ `user_profiles.wallet_public_key`).
   - Copia certificada del *Operating Agreement* y estatutos de la empresa promotora.
   - Certificado de vigencia y cumplimiento (*Certificate of Good Standing*).
3. **Credencial Institucional On-Chain:**
   - Dirección pública de la wallet institucional de la promotora para co-firmas en Bóvedas Squads v4.
   - Historial de Bóvedas Squads desplegadas por la promotora.
4. **Ficha Pública de Reputación y Track Record:**
   - Logotipo corporativo, biografía institucional y sitio web oficial.
   - Años de experiencia en el mercado local y especialidad constructiva (*Single-family Fix & Flip, Multifamily Value-Add, New Construction*).
   - Volumen histórico de proyectos concluidos e importe total de capital administrado.
5. **Estado de Validación KYB & Badges de Confianza:**
   - Estado del perfil: `pending` $ightarrow$ `verified` $ightarrow$ `suspended`.
   - Fecha y auditor responsable del alta (`verified_by`, `verified_at`).
   - Sellos públicos verificados que se inyectan automáticamente en el Tab "Desarrollador" del marketplace.

### B. Doble Superficie Operativa del Perfil Promotora

#### 1. Panel de Backoffice (Customer Success & Compliance)
- Módulo administrativo restringido al equipo interno de BRIDS.
- Permite registrar a una nueva promotora, subir y auditar su documentación mercantil, validar antecedentes y verificar su registro en Sunbiz o secretarías de estado.
- Asigna formalmente el rol `re_developer` a la wallet del manager y genera la invitación de acceso al portal.

#### 2. Portal del Desarrollador (Sponsor Dashboard)
- Vista de autoservicio para el equipo de la promotora.
- Permite actualizar información pública de marca (logo, biografía, redes, proyectos previos concluidos).
- Muestra el estado de verificación inmutable de sus documentos legales (solo lectura una vez aprobados por compliance).
- Presenta la lista consolidada de todos sus desarrollos listados en `marketplace_entries` y el saldo consolidado de sus tesorerías en Squads Protocol.

### C. Protocolo de Incorporación Asistida (Concierge KYB & Setup de Wallet)
Los desarrolladores inmobiliarios operan en el entorno de la construcción y las finanzas analógicas. El equipo de Customer Success ejecuta un protocolo guiado:
1. **Configuración de Custodia Institucional:** Asistencia paso a paso para configurar un dispositivo de firma segura (Ledger) o cuenta institucional compatible con Solana.
2. **Vinculación Oficial de Clave Pública:** Enlace de la wallet verificada al campo `developer_entities.manager_wallet`.
3. **Prueba de Firma (*Dry-Run*):** Simulación guiada en entorno devnet/staging donde el desarrollador ejecuta una firma de prueba en Squads Protocol para familiarizarse con la gobernanza multifirma.

### D. Control de Acceso RBAC y Variables de Estado
El perfil promotora habilita el control de acceso en la plataforma:
- **Identificador de Rol:** `re_developer`.
- **Variables de Estado en Sesión:**
  * `session.role = 're_developer'`
  * `is_developer_verified: true`
  * `developer_entity_id: UUID / TEXT`
- **Protección de Rutas (*Defense-in-Depth*):**
  * **Proxy / Middleware Gate:** Las rutas `/developer/**` interceptan la petición en el borde; si la sesión no posee rol `re_developer` y `is_developer_verified = true`, redirige a `/403`.
  * **Page-Level Verification:** Los Server Components validan la entidad promotora activa antes de renderizar interfaces de creación.
  * **API Handlers:** Todos los endpoints bajo `/api/developer/*` verifican el rol a nivel de controlador, respondiendo con `403 Forbidden` JSON ante intentos no autorizados.
