...
| Client-side request timeout during batch submission | Frontend waits for synchronous confirmation | Poor UX, perceived failure, encourages unsafe retries | High | High |
...
| Client-side request timeout | `signAllTransactions` + async submit + client polling | `components/admin/core-candy-machine-panel.tsx`, `app/api/admin/core-candy-machine/submit/route.ts` | Submit API returns in <2s for large batch |
...
- New threat vectors: Unauthorized admin mutations on compliance cases, missing justification on reject/override decisions, loss of incident context, financial actions by `restricted_aml`/`suspended` wallets
- Mitigations: Admin-only guards, mandatory reasons, audit trails, runtime compliance gate in purchase flows

## OWASP Mapping
- A01 Broken Access Control: Admin role checks, cross-admin mutation prevention
- A02 Cryptographic Failures: SIWS signature verification, nonce replay protection
- A03 Injection: Parameterized SQL, no injection found
- A04 Insecure Design: Idempotency, webhook dedupe, DAS bounded pagination
- A05 Security Misconfiguration: Optional webhook secret, broad CSP (improving)
- A06 Vulnerable Components: Monitored via `npm audit`
- A07 Identification/Auth Failures: SIWS nonce, challenge consumption, idempotency
- A08 Software/Data Integrity: Unique constraints, signature uniqueness, monotonic versioning
- A09 Logging/Monitoring: Purchase flow events, webhook ingestion logs
- A10 SSRF: DAS endpoint validation, devnet-only RPC
