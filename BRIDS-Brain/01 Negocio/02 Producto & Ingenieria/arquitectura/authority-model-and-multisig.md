...
| Prepare public purchase transaction | Buyer wallet signer + backend Candy Guard third-party signer | Backend validates guard payment mode (`tokenPayment`/`solPayment`) and guard signer match before returning transaction to user |
